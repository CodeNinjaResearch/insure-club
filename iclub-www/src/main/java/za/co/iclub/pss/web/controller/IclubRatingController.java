...
public class IclubRatingController implements Serializable {
	...
	public boolean validateForm(boolean flag, boolean bulkSave) {
		...
		if (!bulkSave && bean != null && (bean.getReBaseValue() == null || bean.getReBaseValue().trim().equalsIgnoreCase(""))) {
			...
			ret = ret && false;
		} else if (rateTypeBean != null && rateTypeBean.getRtType().equalsIgnoreCase("L") && bean != null && !bean.getReBaseValue().trim().equalsIgnoreCase("") && rateEngineMap != null && rateEngineMap.get(new Double(bean.getReBaseValue())) != null) {
