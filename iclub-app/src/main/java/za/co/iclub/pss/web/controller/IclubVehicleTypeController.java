...
public class IclubVehicleTypeController implements Serializable {
	...
	private static final String BASE_URL = BUNDLE.getString("ws.protocol") + BUNDLE.getString("ws.host") + ":" + BUNDLE.getString("ws.port") + BUNDLE.getString("ws.context") + "/iclub/IclubVehicleTypeService/";
	...
	public void addIclubVehicleType() {
		...
		try {
			if (validateForm(true)) {
				...
				IclubVehicleTypeModel model = IclubVehicleTypeTrans.fromUItoWS(bean);
	...
	public List<IclubVehicleTypeBean> getBeans() {
		...
		if (models != null && models.size() > 0) {
			for (IclubVehicleTypeModel model : models) {
				IclubVehicleTypeBean bean = IclubVehicleTypeTrans.fromWStoUI(model);
