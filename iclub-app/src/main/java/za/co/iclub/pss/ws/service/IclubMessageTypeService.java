...
public class IclubMessageTypeService {
	...
	public ResponseModel add(IclubMessageTypeModel model) {
		try {
			IclubMessageType iMt = IclubMessageTypeTrans.fromWStoORM(model);
		...
		try {
			...
			if (batmod != null && batmod.size() > 0) {
				for (Object object : batmod) {
					IclubMessageType bean = (IclubMessageType) object;
					
					IclubMessageTypeModel model = IclubMessageTypeTrans.fromORMtoWS(bean);
	...
	public IclubMessageTypeModel getById(@PathParam("id") Long id) {
		...
		try {
			...
			model = IclubMessageTypeTrans.fromORMtoWS(bean);
