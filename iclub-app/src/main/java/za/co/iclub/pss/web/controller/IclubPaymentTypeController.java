...
public class IclubPaymentTypeController implements Serializable {
	...
	private static final String BASE_URL = BUNDLE.getString("ws.protocol") + BUNDLE.getString("ws.host") + ":" + BUNDLE.getString("ws.port") + BUNDLE.getString("ws.context") + "/iclub/IclubPaymentTypeService/";
	...
	public void addIclubPaymentType() {
		...
		try {
			if (validateForm(true)) {
				...
				IclubPaymentTypeModel model = IclubPaymentTypeTrans.fromUItoWS(bean);
	...
	public List<IclubPaymentTypeBean> getBeans() {
		...
		if (models != null && models.size() > 0) {
			for (IclubPaymentTypeModel model : models) {
				IclubPaymentTypeBean bean = IclubPaymentTypeTrans.fromWStoUI(model);
