...
public class IclubDocumentTypeController implements Serializable {
	...
	private static final String BASE_URL = BUNDLE.getString("ws.protocol") + BUNDLE.getString("ws.host") + ":" + BUNDLE.getString("ws.port") + BUNDLE.getString("ws.context") + "/iclub/IclubDocumentTypeService/";
	...
	public void addIclubDocumentType() {
		...
		try {
			if (validateForm(true)) {
				...
				IclubDocumentTypeModel model = IclubDocumentTypeTrans.fromUItoWS(bean);
	...
	public List<IclubDocumentTypeBean> getBeans() {
		...
		if (models != null && models.size() > 0) {
			for (IclubDocumentTypeModel model : models) {
				IclubDocumentTypeBean bean = IclubDocumentTypeTrans.fromWStoUI(model);
