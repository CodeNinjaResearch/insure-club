...
public class IclubCohortController implements Serializable {
	...
	protected static final Logger LOGGER = Logger
			.getLogger(IclubCohortController.class);
	private static final String BASE_URL = BUNDLE.getString("ws.protocol")
			+ BUNDLE.getString("ws.host") + ":" + BUNDLE.getString("ws.port")
			+ BUNDLE.getString("ws.context") + "/iclub/IclubCohortService/";
	private static final String NFT_BASE_URL = BUNDLE.getString("ws.protocol")
			+ BUNDLE.getString("ws.host") + ":" + BUNDLE.getString("ws.port")
			+ BUNDLE.getString("ws.context")
			+ "/iclub/IclubNotificationTypeService/";
	private static final String CI_BASE_URL = BUNDLE.getString("ws.protocol")
			+ BUNDLE.getString("ws.host") + ":" + BUNDLE.getString("ws.port")
			+ BUNDLE.getString("ws.context")
			+ "/iclub/IclubCohortInviteService/";
	private static final String CHT_BASE_URL = BUNDLE.getString("ws.protocol")
			+ BUNDLE.getString("ws.host") + ":" + BUNDLE.getString("ws.port")
			+ BUNDLE.getString("ws.context") + "/iclub/IclubCohortTypeService/";
	private static final String IIT_BASE_URL = BUNDLE.getString("ws.protocol")
			+ BUNDLE.getString("ws.host") + ":" + BUNDLE.getString("ws.port")
			+ BUNDLE.getString("ws.context")
			+ "/iclub/IclubInsuranceItemTypeService/";
	private static final String P_BASE_URL = BUNDLE.getString("ws.protocol")
			+ BUNDLE.getString("ws.host") + ":" + BUNDLE.getString("ws.port")
			+ BUNDLE.getString("ws.context") + "/iclub/IclubPersonService/";
	private static final String IS_BASE_URL = BUNDLE.getString("ws.protocol")
			+ BUNDLE.getString("ws.host") + ":" + BUNDLE.getString("ws.port")
			+ BUNDLE.getString("ws.context")
			+ "/iclub/IclubInviteStatusService/";
	...
	public String cohortSummaryAction() {
		...
		if (cohortId != null && !cohortId.trim().equalsIgnoreCase("")) {
			WebClient client = IclubWebHelper.createCustomClient(BASE_URL
					+ "getCohortSummaryById/" + cohortId);
			IclubCohortSummaryModel model = (IclubCohortSummaryModel) (client
					.accept(MediaType.APPLICATION_JSON)
					.get(IclubCohortSummaryModel.class));
			client.close();
			WebClient userClient = IclubWebHelper.createCustomClient(BASE_URL
					+ "getCohortSummaryById/" + getSessionUserId());
			IclubCohortSummaryModel userModel = (IclubCohortSummaryModel) (userClient
					.accept(MediaType.APPLICATION_JSON)
					.get(IclubCohortSummaryModel.class));
			...
			if (model != null) {
				...
				cohortSummaryBean
						.setClaimSinceI(model.getClaimSinceI() != null ? model
								.getClaimSinceI() : 0.0);
				cohortSummaryBean
						.setClaimsInYear(model.getClaimsInYear() != null ? model
								.getClaimsInYear() : 0.0);
				cohortSummaryBean
						.setPremiumForYear(model.getPremiumForYear() != null ? model
								.getPremiumForYear() : 0.0);
				cohortSummaryBean.setPremiumPaidInYear(model
						.getPremiumPaidInYear() != null ? model
						.getPremiumPaidInYear() : 0.0);
				cohortSummaryBean
						.setPrimumSinceI(model.getPrimumSinceI() != null ? model
								.getPrimumSinceI() : 0.0);
			}
			if (userModel != null) {
				cohortSummaryFlag = true;
				cohortSummaryUserBean
						.setClaimSinceI(model.getClaimSinceI() != null ? model
								.getClaimSinceI() : 0.0);
				cohortSummaryUserBean
						.setClaimsInYear(model.getClaimsInYear() != null ? model
								.getClaimsInYear() : 0.0);
				cohortSummaryUserBean.setPremiumForYear(model
						.getPremiumForYear() != null ? model
						.getPremiumForYear() : 0.0);
				cohortSummaryUserBean.setPremiumPaidInYear(model
						.getPremiumPaidInYear() != null ? model
						.getPremiumPaidInYear() : 0.0);
				cohortSummaryUserBean
						.setPrimumSinceI(model.getPrimumSinceI() != null ? model
			...
			IclubWebHelper.addMessage("Please Enter Cohort Id",
	...
	public List<IclubCohortBean> getDashBoardBeans() {
		WebClient client = IclubWebHelper.createCustomClient(BASE_URL
				+ "/get/user/" + getSessionUserId());
		Collection<? extends IclubCohortModel> models = new ArrayList<IclubCohortModel>(
				client.accept(MediaType.APPLICATION_JSON).getCollection(
						IclubCohortModel.class));
		client.close();
	...
	public String addIclubCohorts() {
		LOGGER.info("Class :: " + this.getClass()
				+ " :: Method :: addIclubCohort");
		try {
			if (validateForm(true)) {

				List<IclubCohortModel> models = new ArrayList<IclubCohortModel>();
				if (selectedBeans != null && selectedBeans.size() == 1
						&& !showCreateCont) {
					for (IclubCohortBean bean : selectedBeans) {
						this.bean = bean;

						WebClient client = IclubWebHelper
								.createCustomClient(P_BASE_URL + "get/"
										+ getSessionUserId());

						IclubPersonModel model = (IclubPersonModel) (client
								.accept(MediaType.APPLICATION_JSON)
								.get(IclubPersonModel.class));
						client.close();
						...
						inviteModel.setCiCrtdDt(new Date(System
								.currentTimeMillis()));
						...
						if (inviteModel.getCiId() == null) {
							...
							client = IclubWebHelper
									.createCustomClient(CI_BASE_URL + "add");
							response = client
									.accept(MediaType.APPLICATION_JSON).post(
											inviteModel, ResponseModel.class);
							client.close();
						} else {
							client = IclubWebHelper
									.createCustomClient(CI_BASE_URL + "mod");
							response = client
						...
						if (response.getStatusCode() == 0) {
							...
							IclubWebHelper.addMessage("Cohort " + " "
					...
					WebClient client = IclubWebHelper
							.createCustomClient(BASE_URL + "add");
					...
					ResponseModel response = client.accept(
							MediaType.APPLICATION_JSON).post(model,
							ResponseModel.class);
					client.close();
					if (response.getStatusCode() == 0) {

						client = IclubWebHelper.createCustomClient(P_BASE_URL
								+ "get/" + getSessionUserId());

						IclubPersonModel person = (IclubPersonModel) (client
								.accept(MediaType.APPLICATION_JSON)
								.get(IclubPersonModel.class));
						...
						response = client.accept(MediaType.APPLICATION_JSON)
								.put(person, ResponseModel.class);
						...
						client = IclubWebHelper.createCustomClient(CI_BASE_URL
								+ "add");
						response = client.accept(MediaType.APPLICATION_JSON)
								.post(inviteModel, ResponseModel.class);
						client.close();
						IclubWebHelper.addMessage(
								getLabelBundle().getString("bankmaster")
										+ " "
										+ getLabelBundle().getString(
												"mod.success"),
								FacesMessage.SEVERITY_INFO);
					...
					IclubWebHelper.addMessage("Select one Row",
							FacesMessage.SEVERITY_INFO);
					return "";
				}
			...
			LOGGER.error(e, e);
			IclubWebHelper.addMessage(getLabelBundle().getString("bankmaster")
		...
		try {
			if (validateForm(true)) {
				...
				if (selectedInviteBeans != null
						&& selectedInviteBeans.size() > 0) {
					WebClient client = IclubWebHelper
							.createCustomClient(CI_BASE_URL + "addList");
					for (IclubCohortInviteBean bean : selectedInviteBeans) {

						IclubCohortInviteModel model = IclubCohortInviteTrans
								.fromUItoWS(bean);
					...
					ResponseModel response = client.accept(
							MediaType.APPLICATION_JSON).postCollection(models,
							IclubCohortInviteModel.class, ResponseModel.class);
					client.close();
					if (response.getStatusCode() == 0) {
						IclubWebHelper.addMessage(
								getLabelBundle().getString("cohortinvite")
										+ " "
										+ getLabelBundle().getString(
												"mod.success"),
								FacesMessage.SEVERITY_INFO);
	...
	public String deRegisterCohort() {
		...
		LOGGER.info("Class :: " + this.getClass()
				+ " :: Method :: deRegisterCohort");
		try {
			WebClient client = IclubWebHelper.createCustomClient(P_BASE_URL
					+ "get/" + getSessionUserId());
			IclubPersonModel model = client.accept(MediaType.APPLICATION_JSON)
					.get(IclubPersonModel.class);

			model.setIclubCohort(null);
			client = IclubWebHelper.createCustomClient(P_BASE_URL + "mod");
			ResponseModel response = client.accept(MediaType.APPLICATION_JSON)
					.put(model, ResponseModel.class);
			if (response.getStatusCode() == 0) {
				IclubWebHelper.addMessage("Cohort " + " "
			...
			LOGGER.error(e, e);
			IclubWebHelper.addMessage(
					"Cohort " + " " + getLabelBundle().getString("del.error")
							+ " :: " + e.getMessage(),
					FacesMessage.SEVERITY_ERROR);
		}
	...
	public void modIclubCohort() {
		...
		try {
			if (validateForm(false)) {
				...
				ResponseModel response = client.accept(
						MediaType.APPLICATION_JSON).put(model,
						ResponseModel.class);
				client.close();
				if (response.getStatusCode() == 0) {
					IclubWebHelper
	...
	public void updatecohort(Long status, String approveOrReject) {
		try {
			...
			if (selecteAdminCohortInviteBeans != null
					&& selecteAdminCohortInviteBeans.size() > 0) {
				List<IclubCohortInviteModel> models = new ArrayList<IclubCohortInviteModel>();
				for (IclubCohortInviteBean bean : selecteAdminCohortInviteBeans) {
					...
					if (status == 3 && bean.getIclubInviteStatus() == 2) {
						WebClient client = IclubWebHelper
								.createCustomClient(P_BASE_URL + "get/"
										+ bean.getIclubPerson());
						IclubPersonModel personModel = client.accept(
								MediaType.APPLICATION_JSON).get(
								IclubPersonModel.class);

						personModel.setIclubCohort(bean.getIclubCohort());
						client = IclubWebHelper.createCustomClient(P_BASE_URL
								+ "mod");
						ResponseModel response = client.accept(
								MediaType.APPLICATION_JSON).put(personModel,
								ResponseModel.class);
						if (response.getStatusCode() == 0) {
				...
				WebClient client = IclubWebHelper
						.createCustomClient(CI_BASE_URL + "modList");
				ResponseModel response = client.accept(
						MediaType.APPLICATION_JSON).postCollection(models,
						IclubCohortInviteModel.class, ResponseModel.class);
				client.close();
				if (response.getStatusCode() == 0) {
					IclubWebHelper.addMessage("CohortInvite" + " "
							+ approveOrReject + " Successfully",
							FacesMessage.SEVERITY_INFO);
					viewParam = 1l;
					showView();
				} else {
					IclubWebHelper.addMessage("CohortInvite" + " "
							+ approveOrReject
							+ "error :: Web Service Error - Contact Admin",
							FacesMessage.SEVERITY_ERROR);
				}
			} else {
				IclubWebHelper.addMessage("Select atleast on record",
						FacesMessage.SEVERITY_INFO);
			}

		} catch (Exception e) {
			LOGGER.error(e, e);
			IclubWebHelper.addMessage("CohortInvite" + " " + approveOrReject
					+ "error :: Web Service Error - Contact Admin",
					FacesMessage.SEVERITY_ERROR);
		}
	...
	public void setIclubCohortInvite(String access_token, String fromSocial,
		...
		try {
			...
			LOGGER.info("Class :: " + this.getClass()
					+ " :: Method :: setIclubCohortInvite");
			LOGGER.info(fromSocial + "=====FromSocial====");
			if (newInvites) {
				access_token = IclubWebHelper.getObjectIntoSession("key")
						.toString();
			}
			if (fromSocial != null && fromSocial.equalsIgnoreCase("OUTLOOK")) {

				HttpClient client = new DefaultHttpClient();
				HttpGet outlookRequest = new HttpGet(
						"https://apis.live.net/v5.0/me/contacts?access_token="
								+ access_token);
				HttpResponse response = client.execute(outlookRequest);
				String outputString = EntityUtils
						.toString(response.getEntity());

				OutlookContactDataBean data = new Gson().fromJson(outputString,
						OutlookContactDataBean.class);
				...
				if (data != null && data.getData() != null
					...
					for (OutLookContactsBean contact : data.getData()) {
						...
						bean.setCiInviteUri(contact.getEmails() != null ? contact
								.getEmails().getPreferred() : contact
								.getPhones() != null ? contact.getPhones()
								.getPreferred() : null);
						if (bean.getCiInviteUri() != null
								&& !bean.getCiInviteUri().trim()
										.equalsIgnoreCase("")) {
							cohortsInviteBeanMap.put(bean.getCiInviteUri(),
									bean);
						}
					}
				...
				String redirectUrl = "https://www.facebook.com/dialog/apprequests?app_id="
						+ BUNDLE.getString("fb.client_id")
						+ "&redirect_uri="
						+ BUNDLE.getString("fb.app_redirect_uri")
						+ "&message="
						+ BUNDLE.getString("fb.message");
				try {
					IclubWebHelper.addObjectIntoSession("access_token",
							access_token);
					IclubWebHelper.addObjectIntoSession("fromSocial",
							fromSocial);
			...
			} else if (fromSocial != null
					&& fromSocial.equalsIgnoreCase("fbapp")) {
				boolean flag = true;
				int i = 0;
				while (flag) {
					...
					&& fromSocial.equalsIgnoreCase("yahoo")) {
				HttpClient client = new DefaultHttpClient();
				String callUrl1 = "https://social.yahooapis.com/v1/user/me"
				...
				String outputString = EntityUtils.toString(response2
						.getEntity());
				JsonObject jsonGet = (JsonObject) new JsonParser()
						.parse(outputString);
				if (jsonGet.has("contacts")) {
					jsonGet = (JsonObject) new JsonParser().parse(jsonGet.get(
							"contacts").toString());
					ObjectMapper mapper = new ObjectMapper();
					List<YahooContactBean> contactBeans = mapper.readValue(
							jsonGet.get("contact").toString(), TypeFactory
									.collectionType(List.class,
											YahooContactBean.class));
					...
					if (contactBeans != null && contactBeans.size() > 0) {
						for (YahooContactBean contactBean : contactBeans) {
							...
							bean.setIclubNotificationType(3l);
							bean.setCiId(UUID.randomUUID().toString());
							if (contactBean.getFields() != null
									&& contactBean.getFields().size() > 0) {
								for (YahooFieldBean fBean : contactBean
										.getFields()) {
									if (fBean.getType() != null
											&& fBean.getType()
													.equalsIgnoreCase("email")) {
										bean.setCiInviteUri(fBean.getValue()
												.toString());
										break;
									} else if (fBean.getType() != null
											&& fBean.getType()
				...
				for (ContactEntry entry : resultFeed.getEntries()) {
					...
					bean.setIclubNotificationType(3l);
					bean.setCiInviteAcceptYn("N");
					bean.setCiInviteFName(entry.getName() != null ? entry
							.getName().getFullName() != null ? entry.getName()
							.getFullName().getValue() : null : null);
					if (bean.getCiInviteUri() == null
							|| bean.getCiInviteUri().trim()
									.equalsIgnoreCase("")) {
						if (entry.hasName()) {
							if (entry.hasPhoneNumbers()) {
								for (PhoneNumber phnum : entry
										.getPhoneNumbers()) {
			...
			if (cohortsInviteBeanMap != null && cohortsInviteBeanMap.size() > 0) {
				...
				Collection<? extends String> existingNumbers = client.accept(
						MediaType.APPLICATION_JSON).postAndGetCollection(
						cohortsInviteBeanMap.keySet(), String.class,
						String.class);
				client.close();
				...
				Collection<? extends String> existingEmials = client.accept(
						MediaType.APPLICATION_JSON).postAndGetCollection(
						cohortsInviteBeanMap.keySet(), String.class,
						String.class);
				client.close();
				...
				if (newInvites) {
					...
					Collection<? extends String> notInvitedEmials = client
							.accept(MediaType.APPLICATION_JSON)
							.postAndGetCollection(
									cohortsInviteBeanMap.keySet(),
				...
				if (cohortsInviteBeanMap.size() > 0) {
					...
					if (fromSocial != null
						...
						for (IclubCohortInviteBean bean : cohortsInviteBeans) {
							...
							model.setIclubCohort(this.bean.getCId());
							model.setCiCrtdDt(new Date(System
									.currentTimeMillis()));
							models.add(model);

						}
						client.accept(MediaType.APPLICATION_JSON)
								.postCollection(models,
										IclubCohortInviteModel.class,
										ResponseModel.class);
						client.close();
					}
			...
			e.printStackTrace();
			LOGGER.error(e, e);
			IclubWebHelper.addMessage("Fail :: " + e.getMessage(),
					FacesMessage.SEVERITY_ERROR);
		}
	}
	...
	public String getSessionUserId() {
		Object sessUsrId = IclubWebHelper.getObjectIntoSession(BUNDLE
				.getString("logged.in.user.id"));
		if (sessUsrId == null)
			sessionUserId = "1";
		else
	...
	public String getUserName() {
		userName = IclubWebHelper.getObjectIntoSession(
				BUNDLE.getString("logged.in.user.scname")).toString();
		return userName;
	}
	...
	public List<IclubPersonBean> getPersonBeans() {
		WebClient client = IclubWebHelper.createCustomClient(P_BASE_URL
				+ "list");
		Collection<? extends IclubPersonModel> models = new ArrayList<IclubPersonModel>(
				client.accept(MediaType.APPLICATION_JSON).getCollection(
						IclubPersonModel.class));
		client.close();
	...
	public List<IclubCohortTypeBean> getCohortTypeBeans() {
		...
		WebClient client = IclubWebHelper.createCustomClient(CHT_BASE_URL
				+ "list");
		Collection<? extends IclubCohortTypeModel> models = new ArrayList<IclubCohortTypeModel>(
				client.accept(MediaType.APPLICATION_JSON).getCollection(
						IclubCohortTypeModel.class));
		client.close();
		cohortTypeBeans = new ArrayList<IclubCohortTypeBean>();
		if (models != null && models.size() > 0) {
			for (IclubCohortTypeModel model : models) {

				IclubCohortTypeBean bean = IclubCohortTypeTrans
						.fromWStoUI(model);
	...
	public void setCohortsInviteBeans(
			List<IclubCohortInviteBean> cohortsInviteBeans) {
	...
	public void setSelectedInviteBeans(
			List<IclubCohortInviteBean> selectedInviteBeans) {
		this.selectedInviteBeans = selectedInviteBeans;
	}

	public List<IclubNotificationTypeBean> getIclubNotificationTypeBeans() {
		WebClient client = IclubWebHelper.createCustomClient(NFT_BASE_URL
				+ "list");
		Collection<? extends IclubNotificationTypeModel> models = new ArrayList<IclubNotificationTypeModel>(
				client.accept(MediaType.APPLICATION_JSON).getCollection(
						IclubNotificationTypeModel.class));
		client.close();
		iclubNotificationTypeBeans = new ArrayList<IclubNotificationTypeBean>();
		if (models != null && models.size() > 0) {
			for (IclubNotificationTypeModel model : models) {

				IclubNotificationTypeBean bean = IclubNotificationTypeTrans
						.fromWStoUI(model);
	...
	public void setIclubNotificationTypeBeans(
			List<IclubNotificationTypeBean> iclubNotificationTypeBeans) {
	...
	public String getKey() {
		...
		if (request.getParameter("key") != null) {
			...
			if (IclubWebHelper.getObjectIntoSession("newInvites") != null) {
				newInvites = new Boolean(IclubWebHelper.getObjectIntoSession(
						"newInvites").toString());
			}
			...
			if (IclubWebHelper.getObjectIntoSession("cohortInviteId") != null
					|| IclubWebHelper.getObjectIntoSession("cohortId") != null) {
				try {
					...
					if (IclubWebHelper.getObjectIntoSession("cohortId") != null) {
						cohortId = IclubWebHelper.getObjectIntoSession(
								"cohortId").toString();
					}
					if (cohortId == null
							|| cohortId.trim().equalsIgnoreCase("")) {
						client = IclubWebHelper.createCustomClient(CI_BASE_URL
								+ "get/"
								+ IclubWebHelper.getObjectIntoSession(
										"cohortInviteId").toString());
						inviteModel = client.get(IclubCohortInviteModel.class);
						inviteModel.setCiInviteAcceptYn("Y");
						client = IclubWebHelper.createCustomClient(CI_BASE_URL
								+ "mod");
						responseModels = client.accept(
								MediaType.APPLICATION_JSON).put(inviteModel,
								ResponseModel.class);
						cohortId = inviteModel.getIclubCohort();
						IclubWebHelper.addObjectIntoSession("cohortInviteId",
								null);
					}

					if ((responseModels != null && responseModels
							.getStatusCode() == 0)
							|| (cohortId != null && !cohortId.trim()
									.equalsIgnoreCase(""))) {
						client = IclubWebHelper.createCustomClient(BASE_URL
								+ "get/" + cohortId);
						IclubCohortModel model = client
								.get(IclubCohortModel.class);
						IclubCohortBean bean = IclubCohortTrans
								.fromWStoUI(model);
						selectedBeans = new ArrayList<IclubCohortBean>();
						selectedBeans.add(bean);
						...
						navigationHandler.handleNavigation(
								...
								"/pages/admin/cohorts/" + addIclubCohorts());
					}
				} catch (Exception e) {
				...
				&& request.getParameter("from").toString()
						.equalsIgnoreCase("fbapp")) {
			String access_token = IclubWebHelper
					.getObjectIntoSession("access_token") != null ? IclubWebHelper
					.getObjectIntoSession("access_token").toString() : null;
			String guid = IclubWebHelper.getObjectIntoSession("guid") != null ? IclubWebHelper
					.getObjectIntoSession("guid").toString() : null;
	...
	public void setCohortSummaryUserBean(
			IclubCohortSummaryBean cohortSummaryUserBean) {
	...
	public void setNewCohortsInviteBeans(
			List<IclubCohortInviteBean> newCohortsInviteBeans) {
	...
	public List<IclubInsuranceItemTypeBean> getIclubInsuranceItemTypeBeans() {
		...
		WebClient client = IclubWebHelper.createCustomClient(IIT_BASE_URL
				+ "list");
		Collection<? extends IclubInsuranceItemTypeModel> models = new ArrayList<IclubInsuranceItemTypeModel>(
				client.accept(MediaType.APPLICATION_JSON).getCollection(
						IclubInsuranceItemTypeModel.class));
		client.close();
		iclubInsuranceItemTypeBeans = new ArrayList<IclubInsuranceItemTypeBean>();
		if (models != null && models.size() > 0) {
			for (IclubInsuranceItemTypeModel model : models) {

				IclubInsuranceItemTypeBean bean = IclubInsuranceItemTypeTrans
						.fromWStoUI(model);
	...
	public void setIclubInsuranceItemTypeBeans(
			List<IclubInsuranceItemTypeBean> iclubInsuranceItemTypeBeans) {
		this.iclubInsuranceItemTypeBeans = iclubInsuranceItemTypeBeans;
	}

	public List<IclubCohortInviteBean> getAdminCohortInviteBeans() {

		WebClient client = IclubWebHelper.createCustomClient(CI_BASE_URL
				+ "admin/list/" + getSessionUserId());
		Collection<? extends IclubCohortInviteModel> models = new ArrayList<IclubCohortInviteModel>(
				client.accept(MediaType.APPLICATION_JSON).getCollection(
						IclubCohortInviteModel.class));
		client.close();
		...
		if (models != null && models.size() > 0) {
			for (IclubCohortInviteModel model : models) {
				...
				IclubCohortInviteBean bean = IclubCohortInviteTrans
						.fromWStoUI(model);
	...
	public void setAdminCohortInviteBeans(
			List<IclubCohortInviteBean> adminCohortInviteBeans) {
		this.adminCohortInviteBeans = adminCohortInviteBeans;
	}

	public List<IclubInviteStatusBean> getInviteStatusBeans() {
		WebClient client = IclubWebHelper.createCustomClient(IS_BASE_URL
				+ "list");
		Collection<? extends IclubInviteStatusModel> models = new ArrayList<IclubInviteStatusModel>(
				client.accept(MediaType.APPLICATION_JSON).getCollection(
						IclubInviteStatusModel.class));
		client.close();
		inviteStatusBeans = new ArrayList<IclubInviteStatusBean>();
		if (models != null && models.size() > 0) {
			for (IclubInviteStatusModel model : models) {

				IclubInviteStatusBean bean = IclubInviteStatusTrans
						.fromWStoUI(model);
	...
	public void setInviteStatusBeans(
			List<IclubInviteStatusBean> inviteStatusBeans) {
	...
	public void setSelecteAdminCohortInviteBeans(
			List<IclubCohortInviteBean> selecteAdminCohortInviteBeans) {
